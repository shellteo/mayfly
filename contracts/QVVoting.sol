...
contract QVVoting is Ownable, MinterRole {
    ...
    event test(uint prev, uint next);
    ...
    */
    ...
    function updateVotes(uint numTokens) public {
        _balances[msg.sender] = _balances[msg.sender].add(numTokens);
    }
    function testVotes() public {
        uint a = _balances[msg.sender];
        _balances[msg.sender] = _balances[msg.sender].sub(1);
        emit test(a, _balances[msg.sender]);
        // return (a, _balances[msg.sender]);
    }
